...
class JiraWatchIssueCommand extends JiraBaseCommand 
{
    protected $signature = 'wi {issue : The issue to set a due date on} {--user= : The user to watch as}';
    protected $description = 'Watch and issue';
    ...
    public function handle()
    {
        ...
        if(!in_array($user, $currentWatchers)) 
        {
            ...
            $this->line("Adding " . $user . " to watchers");
            try 
            {
                app(Jira::class)->setIssueWatchers($issueKey, array_merge($currentWatchers, [$user]));
            } 
            catch(\Exception $ex) 
            {
        ...
        {
            $this->error($user . " is already watching " . $issueKey);
        }
    }
}
